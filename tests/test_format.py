...
def test_stylish_1():
    with open('tests/fixtures/format_stylish.txt') as data:
        ...
        assert stylish.render(input_check_1) == result[0]


def test_stylish_2():
    with open('tests/fixtures/format_stylish.txt') as data:
        result = data.read().strip().split('\n\n\n')
        assert stylish.render(input_check_2) == result[1]


def test_plain_1():
    with open('tests/fixtures/format_plain.txt') as data:
        result = data.read().strip().split('\n\n\n')
        assert plain.render(input_check_1) == result[0]


def test_plain_2():
    with open('tests/fixtures/format_plain.txt') as data:
        result = data.read().strip().split('\n\n\n')
        assert plain.render(input_check_2) == result[1]


def test_jsonify_1():
    with open('tests/fixtures/format_json.txt') as data:
        assert jsonify.render(input_check_1) == data.read().strip()
